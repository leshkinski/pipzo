...
The goal is a calm touchscreen-first bedside player: no phone for playback, no general desktop access, appliance-like kiosk operation, large touch targets, simple Spotify library browsing on Home, Bluetooth speaker output, and a low-light bedtime mode. V1 defaults to the Chromium app-maximized kiosk path so Raspberry Pi OS Squeekboard works for setup/recovery text entry; true fullscreen kiosk remains available as an explicit override when touch keyboard input is not needed.
